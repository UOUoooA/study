  ...
  <div class="trade-container">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{findUserDefault.fullAddress}}</span>
        收货人：<span>{{findUserDefault.consignee}}</span>
        <span>{{findUserDefault.phoneNum}}</span>
      </div>
    </div>
...
export default {
  ...
  computed:{
      ...mapState({
        addressInfo:state=>state.trade.userInfoList,
  ...
  methods:{
    changeisDefault(address1,address2){
      address2=Array.from(address2)
      address2.forEach(item=>item.isDefault=0)
     ...
     let res = await this.$API.reqSendOrder(tradeNo,data)
      if(res.code==200){
