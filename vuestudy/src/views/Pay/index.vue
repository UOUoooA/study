<template>
  <div class="pay-main">
    <el-button type="primary">主要按钮</el-button>    <div class="pay-container">
...
<script>
  ...
  export default {
    ...
    methods:{
     async getPayInfo(){
      let res = await this.$API.reqPaymessage(this.orderId)
     if(res.code==200){
     ...
     async open() {
       ...
       if(!this.timer){
        ...
        let res = await this.$API.reqCheckPayInfo(this.orderId)
           console.log(res)
           if(res.code==200){
             clearInterval(this.timer)
             this.timer=null
